...
class Engine extends EventEmitter {
    ...
    decide(data) { //No going backwards in the conversation
        ...
        this.log.debug('Decided on next state', availableStates[0] || session.__engine__.state);
    ...
    move(data, state) {
        ...
        if (selectesState && selectesState.visitCount < selectesState.maxVisits) {
            let from = session.__engine__.state;
            session.__engine__.state = state;
            this.emit('move', session, from, state);
            selectesState.visitCount++;
            this.emit(state.name, data, (data) => {
                this.postMove(data);
            });
        } else {
            this.emit('error', new Error('Infinite loop error or state not found'), state && state.name);
        }
    }
